...
#def test(epoch,dataloader, model, criterion1,  opt,writer,log_file, is_test=True):
def test(epoch,dataloader, model, opt,writer,log_file, is_test=True):#zhao delete 'criterion1'
    model.eval()
    ...
    Acc_class_Record = np.zeros((21, 11, 21))
    ...
    with torch.no_grad():
        ...
        for i, (edge_features, node_features, adj_mat, graph_label) in enumerate(dataloader, 0):
            ...
            '''
            if opt.featdir:
                groundtruth = torch.argmax(labels, dim=2) # [batch,seq_size,1]
            else:
                groundtruth = None
            '''
            if opt.loss==1:
                pre_out = model(init_input, edge_states, adj_matrixes, true_lengths, is_test=True)
            else:
                pre_out = model(init_input, edge_states, adj_matrixes, true_lengths, is_test=True)
            ...
            if not log_file.endswith('txt'):
                for j in range(len(gt_labels)):
                    ...
                    for seq in range(10):
                        Acc_class_Record[labell, seq, pre_label[j,seq,0]] += 1
            ...
            if opt.loss==1:
                ...
                loss =  loss  # TODO need regularzation??
        ...
        if log_file.endswith('txt'):
            ...
            Acc_class_Record[:] = Acc_class_Record[:] / VidClass_CNT.reshape(-1,1,1)
            Acc_Record[:, -1] = Acc_Record[:, :-1].mean(-1)
            np.savetxt(log_file+'/2.csv', Acc_Record, delimiter=',')
            if if_ratio:
                for k in range(10):
                    np.savetxt(log_file+'/_confusionM_ratio{}.csv'.format(k+1), Acc_class_Record[:,k,:], delimiter=',')
            else:
                np.savetxt(log_file+'/_confusionM.csv', Acc_class_Record.mean(-2), delimiter=',')
            np.save(log_file+'/_AccClassRec.npz', Acc_class_Record)
        np.savetxt(opt.logroot+'/result_st_output.csv',output,delimiter=',')
