...
def read_features(segments_feature_path, filename):
    ...
    with open(filename) as f:
        ...
        adj_mat = np.zeros((1+object_num, edge_num))  # adj_mat for GGNN
        node_labels = np.ones((1+object_num)) * -1
        one_hot_graph_label = np.zeros((1,subactivities_label_num))
        ...
        one_hot_graph_label[0, int(line.strip().split(' ')[0])-1] = 1
        colon_seperated = [x.strip() for x in line.strip().split(' ')]
        ...
        for _ in range(object_object_num):
            ...
            #print object_num
                ...
                adj_mat[o1_id][o1_id*(1+object_num)+o2_id] = 1
                #print filename_base
                #print o1_id
                #print o2_id
...
def collect_data(paths):
    ...
    pickle.dump(data, open(os.path.join(paths.tmp_root, 'cad120_data_reco.p'), 'wb'))
