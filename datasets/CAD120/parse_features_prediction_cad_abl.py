...
def collect_data(paths):
    ...
    for sequence_path_file in os.listdir(segments_files_path):
        ...
        with open(os.path.join(segments_files_path, sequence_path_file)) as f:
            ...
            lines = f.readlines()
            if segment_feature_num>max:
                max = segment_feature_num
            if segment_feature_num<5:
                count = count+1
                print(sequence_id)
    ...
    if not os.path.exists(paths.abl_root):
        os.makedirs(paths.abl_root)
    pickle.dump(check_list, open(os.path.join(paths.abl_root, 'check.p'), 'wb'))
    pickle.dump(data, open(os.path.join(paths.abl_root, 'cad120_data_pred.p'), 'wb'))
    pickle.dump(subject_ids, open(os.path.join(paths.abl_root, 'cad120_data_list.p'), 'wb'))
