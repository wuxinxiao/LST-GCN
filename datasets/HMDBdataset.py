...
def data_sampling(img_list, rgb_dir, flow_dir, video_name):
    '''
    ...
    if rgb_dir==flow_dir:
        ...
        flow_feat_path = flow_dir + video_name[:-4]

    imgList = os.listdir(video_frame_path)
    img_index = sample(imgList,10)

    rgbList = os.listdir(rgb_feat_path)
    rgb_index = sample(rgbList,10)

    flowList = os.listdir(flow_feat_path)
    flow_index = sample(flowList,10)
...
class dataset(Dataset):
    ...
    def __init__(self, datalist, detect_path, video_frame_dir,rgb_feat_dir,flow_feat_dir, opt):
        ...
        for line in lines:
            ...
            img_idx,rgb_feat_idx,flow_feat_idx = data_sampling(video_frame_dir,rgb_feat_dir,flow_feat_dir, video_name)
    ...
    def __getitem__(self, index):
        ...
        for i in range(len(imgInd)):  # seq_size
            ...
            #adj_matrix = create_adjacency_matrix_abl(edges, len(nodes),video_node_num) #for GGNN
