...
def get_feature(feat_path):
    ...
    action_24 = [ 'Basketball', 'BasketballDunk', 'Biking', 'CliffDiving', 'CricketBowling', 'Diving', 'Fencing',
    ...
    if  feat_category=='resnet50_rgbflow_same_fintuned':
        ...
        #labels = feat_info['graph_label']
        #print(name)
        video_label = action_24.index(name)
        #print(video_label)
        labels = np.zeros(24+1)#classNum + 1
        labels[video_label] = 1
        labels[-1] = 10
        labels = np.expand_dims(labels,0).repeat(10,0)
        #print(labels.shape)
        #exit()
        return adj_matrixes,nodes_features,edges_features,scene_features,labels
