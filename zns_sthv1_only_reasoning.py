...
from utils.zns_sthv1_train import train
from utils.zns_sthv1_test import test
...
import zns_sthv1_only_reasoning_model
...
def main(opt):
    ...
    model = zns_sthv1_only_reasoning_model.Model(opt)
    #print(model)
    #exit()
    ...
    for epoch_i in range(opt.epoch, opt.niter):
        ...
        train(epoch_i, train_dataloader, model, optimizer, opt, writer, log_file_name)
        ...
        test_acc, output = test(epoch_i,test_dataloader, model, opt, writer, log_file_name, is_test=True)
