...
class TemporalRelationGraph(torch.nn.Module):
    def __init__(self, opt):
        ...
        self.gc = GraphConvSparse(opt.state_dim + opt.tem_feat_dim, opt.gcn_dim1,activation=F.relu)
    ...
    def forward(self, graph_out, init_input, is_test):
        ...
        for seq_ind in range(1,self.seq_size+1):   
            ...
            avg_predict = self.bn(predict_fullgraph)
...
class Model(nn.Module):
    def __init__(self, opt):
        ...
        self.linear_node = nn.Sequential(
            nn.Linear(self.node_feat_dim, self.state_dim * 2),
            nn.ReLU(),
    ...
    def forward(self, init_input, edge_states, A, true_lengths, tem_features,is_test): # init_input:[batch*seq,n_nodes,state_dim]
        ...
        without_spatial = False#False: complete model True£ºwithout reasoning
        if without_spatial:
            ...
            output = self.classifier2(graph_output)
            mse_loss = 0
            mmd_loss = 0
            """
