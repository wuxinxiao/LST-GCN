...
class Semantic(nn.Module):
    ...
    def __init__(self, opt):
        ...
        self.semantic_dim = opt.semantic_dim
        self.seq_size = opt.seq_size
        self.projection_layer1 = nn.Linear(self.state_dim, self.project_dim)
        self.projection_layer2 = nn.Linear(self.state_dim, self.project_dim)
        self.projection_layer3 = nn.Linear(self.state_dim, self.project_dim)
        self.embedding = nn.Sequential(
            nn.Linear(3*self.project_dim, self.semantic_dim),
    ...
    def forward(self, nodes_feats, edges_feats, adj_mat, true_lengths):
        ...
        for batch_ind in range(batch):
            ...
            for seq_i in range(true_length):
                ...
                index1, index2 = np.argwhere(adj_mat[batch_ind, seq_i, :, :] == 1)
                ...
                for i in range(relation_num):
                    ...
                    losses.append(self.vis_loss((f_ij+f_i),f_j).item())
...
class PredictGraph(torch.nn.Module):
    ...
    def __init__(self, opt):
        ...
        input_dim=self.n_cluster*self.state_dim+self.state_dim+self.tem_feat_dim
    ...
    def forward(self, current_nodes):
        ...
        predict_node = self.predic_node(current_nodes,recovered_A)
        
        '''generate predicted sub-graph'''
        #only generate one node
...
class TemporalRelationGraph(torch.nn.Module):
    def __init__(self, opt):
        ...
        self.linear = nn.Sequential(
                        nn.Linear(self.n_cluster*self.state_dim+self.state_dim+self.tem_feat_dim, self.state_dim),
                        nn.ReLU(inplace=True),
                        )
        self.Linear = nn.Sequential(
        ...
        self.graph_rep_part = GenerateGraphRep(opt,self.n_cluster*self.state_dim+self.state_dim+self.tem_feat_dim)
        self.graph_rep_full = GenerateGraphRep(opt, self.n_cluster*self.state_dim+self.state_dim+self.tem_feat_dim)
    ...
    def forward(self, graph_out, init_input, is_test):
        ...
        true_graph_rep =  self.graph_rep_full(new_input)  # true graph representation based on the complete video
        ...
        for seq_ind in range(4, 5):   
            ...
            for scaleID in range(len(new_scales)):
                ...
                for idx in idx_relations_randomsample:
                    ...
                    if  self.training and seq_ind< self.seq_size: 
