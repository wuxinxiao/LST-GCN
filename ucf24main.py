...
from utils.ucf_train import train 
from utils.ucf_test import test 
from utils.val import val
#from datasets.UCF101dataset_general import dataset
from datasets.UCF101_24datasets import dataset
...
parser.add_argument('--batch_size', type=int, default=48, help='input batch size')#48
...
parser.add_argument('--classNum', type=int, default=24, help='number of classes')
...
parser.add_argument('--num_frames', type=int, default=5, help='number of sampled frames in each segment ')###5 8
parser.add_argument('--n_steps', type=int, default=5, help='propogation steps number of GGNN')#3!!
parser.add_argument('--niter', type=int, default=500, help='number of epochs to train for')#50 150 300 500!
...
parser.add_argument('--resume', default='/home/mcislab/zhaojw/AAAI/prediction2020/models/ucf101-24_res50_rgbflow_same_fintuned/num_frames5_lr0.0005_batch48/test/',help='path to latest checkpoint')
parser.add_argument('--logroot', default='/home/mcislab/zhaojw/AAAI/prediction2020/log/ucf101-24_res50_rgbflow_same_fintuned/num_frames5_lr0.0005_batch48/test/',help='path to latest log')
...
def main(opt):
    ...
    train_datalist = '/home/mcislab/zhaojw/AAAI/ucf24list/ucf101-24_train_split' + opt.split +'.txt'
    test_datalist = '/home/mcislab/zhaojw/AAAI/ucf24list/ucf101-24_val_split'+opt.split+'.txt'
    ...
    train_dataset = dataset(train_datalist, paths.resnet50_ucf_rgbflow_same_fintuned,opt)####zhao changed ###paths.resnet50_ucf_rgbflow_same,resnet50_ucf_rgbflow_same_fintuned,ap_resnet18_ucf
    ...
    test_dataset = dataset(test_datalist, paths.resnet50_ucf_rgbflow_same_fintuned,opt)###paths.resnet50_ucf_rgbflow_same
    test_dataloader = DataLoader(test_dataset, batch_size=opt.batch_size, shuffle=True, num_workers=opt.workers, drop_last=False)

    model = UCFModel.Model(opt)
    ...
    split1 = '/home/mcislab/zhaojw/AAAI/prediction2020/models/ucf101_res50_rgbflow_same_fintuned/num_frames5_lr0.0005/2(bn_changed)/split1/35/model_best.pth'
    split2 = '/home/mcislab/zhaojw/AAAI/prediction2020/models/ucf101_res18_rgbflow_same_fintuned/num_frames5_lr0.0005/split2/28/model_best.pth'
    split3 = '/home/mcislab/zhaojw/AAAI/prediction2020/models/ucf101_res50_rgbflow_same_fintuned/num_frames5_lr0.0005/split3/2736/model_best.pth'
    print("load last experiment best model")
    model.load_state_dict(torch.load(split3)['state_dict'])
    test_acc, output = test(0,test_dataloader, model, criterion1, criterion2, opt, writer, log_file_name, is_test=True)
    exit()
    '''
    ...
    for epoch_i in range(opt.epoch, opt.niter):
        ...
        tmp_test_acc = np.mean(test_acc)
        #tmp_test_acc = test_acc[0][4]#save the model where the accuracy is best at ratio 0.5
